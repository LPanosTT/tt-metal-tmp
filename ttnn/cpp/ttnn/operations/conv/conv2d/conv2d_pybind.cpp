...
void py_bind_conv2d(py::module& module) {
    ...
    module.def(
        "prepare_conv_weights_channels_last_for_ttnn",
        [](const ttnn::Tensor& weight_tensor,
            uint32_t in_channels,
            uint32_t out_channels,
            uint32_t batch_size,
            ...
            std::optional<const Conv2dConfig> conv_config_) -> ttnn::Tensor {
                return ttnn::operations::conv::conv2d::prepare_conv_weights_channels_last_for_ttnn(
                    weight_tensor, in_channels, out_channels, batch_size, input_height, input_width, kernel_size, stride, padding, dilation, groups, device, conv_config_);
        },
        py::kw_only(),
        py::arg("weight_tensor"),
        py::arg("in_channels"),
